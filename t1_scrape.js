function(context, args) // target:#s.amal_robo.public regex:"_jr_"
{
    ...
    let news_page = main_page[main_page.length-1].split(/ ?\| ?/g)[0]
    ...
    let members = new Set();
    while(keyVals["project"] = Array.from(projects).pop())
    {
        ...
        let list = temp.split('\n');
        list.filter(i => !i.includes('<') && !i.includes(' '))
            .map(i => {
                let lv = #fs.scripts.get_level({name:i})
                if (typeof(lv) == "number" && lv >= 4) {
                    members.add(i);
                }
            });
    }
    ...
    if (args.regex) {
        ...
        return Array.from(members).filter(x=>regexp.test(x))
    }
    return Array.from(members).filter(x=>x).sort();
}